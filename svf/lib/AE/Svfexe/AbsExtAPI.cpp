...
void AbsExtAPI::handleExtAPI(const CallICFGNode *call)
{
    ...
    for (const std::string &annotation: ExtAPI::getExtAPI()->getExtFuncAnnotations(fun))
    {
        ...
        if (annotation.find("STRCAT") != std::string::npos)
            extType =  STRCAT;
        if (annotation.find("napi_get_cb_info") != std::string::npos)
            extType = NAPI_GET_CB_INFO;
    }
    ...
    else if (extType == NAPI_GET_CB_INFO)
    {
        handleNapiGetCbInfo(call);
    }
    else
    {
...
void AbsExtAPI::handleNapiGetCbInfo(const CallICFGNode *call) {
    AbstractState& as = getAbsStateFromTrace(call);

    // 获取参数
    // const SVFVar* env = call->getArgument(0);
    const SVFVar* cbinfo = call->getArgument(1);
    const SVFVar* argcVar = call->getArgument(2);
    const SVFVar* argvVar = call->getArgument(3);
    const SVFVar* thisArgVar = call->getArgument(4);
    const SVFVar* dataVar = call->getArgument(5);

    // 处理 argc
    if (argcVar) {
        u32_t argc_id = argcVar->getId();
        if (as.inVarToValTable(argc_id)) {
            // 获取 argc 的值
            IntervalValue argcInterval = as[argc_id].getInterval();
            u32_t argc = argcInterval.lb().getIntNumeral();

            // 处理 argv
            if (argvVar) {
                u32_t argv_id = argvVar->getId();
                u32_t cbinfo_id = cbinfo->getId();

                // 假设 cbinfo 是一个数组，将 cbinfo 的 argc 个元素赋值给 argv
                for (u32_t i = 0; i < argc; ++i) {
                    u32_t src_id = cbinfo_id + i;
                    u32_t dst_id = argv_id + i;
                    if (as.inVarToValTable(src_id)) {
                        as[dst_id] = as[src_id];
                    }
                }
            }
    ...
    // 处理 thisArg
    if (thisArgVar) {
        u32_t thisArg_id = thisArgVar->getId();
        u32_t cbinfo_id = cbinfo->getId();
        if (as.inVarToValTable(cbinfo_id)) {
            as[thisArg_id] = as[cbinfo_id];
        }
    }

    // 处理 data
    if (dataVar) {
        u32_t data_id = dataVar->getId();
        u32_t cbinfo_id = cbinfo->getId();
        if (as.inVarToValTable(cbinfo_id)) {
            as[data_id] = as[cbinfo_id];
        }
    }
}
